...
export function KnowledgePage() {
  ...
  const [refluxModalOpen, setRefluxModalOpen] = useState(false);
  const [refluxSelected, setRefluxSelected] = useState<Set<string>>(new Set());
  const [refluxLoading, setRefluxLoading] = useState(false);
  const [refluxResult, setRefluxResult] = useState<{ success: number; failed: number; errors: string[] } | null>(null);
  ...
  const handleReflux = async () => {
    ...
    setUserAccountError('');
    setRefluxLoading(true);
    setRefluxResult(null);
    const result = await refluxSources(currentKb.id, Array.from(refluxSelected), trimmed);
    setRefluxResult(result);
    setRefluxLoading(false);
    setRefluxSelected(new Set());
  };
  ...
  return (
    <div className="flex flex-col h-full bg-background">
                ...
                {(activeTab === 'sources' && sources.length > 0 || activeTab === 'pages' && wikiPages.length > 0) && (
                  batchMode === activeTab ? (
                    ...
                    <div className="flex items-center gap-1">
                      {activeTab === 'sources' && (
                        <button
                          onClick={() => { if (currentKb) loadSources(currentKb.id); setRefluxModalOpen(true); }}
                      ...
                      {sources.map((source) => (
                        <div key={source.id} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-secondary transition-colors">
                          ...
                          {source.refluxStatus === 'processing' ? (
                            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400">
                              <Loader2 className="w-3 h-3 animate-spin" />
                              {t('kb.refluxStatus.processing')}
                            </span>
                          ) : (
                            <span
                            ...
                            >
                              {source.wasRefluxed ? t('kb.refluxStatus.refluxed') : t('kb.refluxStatus.notRefluxed')}
                            </span>
                          )}
                        </div>
                      ))}
      ...
      {/* Reflux Modal - Minimized (floating, no backdrop) */}
      {refluxModalOpen && currentKb && refluxMinimized && (
        <div className="fixed bottom-4 right-4 z-50">
          <div className="bg-background border rounded-lg shadow-lg w-72 flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b">
              <h3 className="text-sm font-medium">{t('kb.batchReflux')}</h3>
              <div className="flex items-center gap-1">
                <button onClick={() => setRefluxMinimized(false)} className="text-muted-foreground hover:text-foreground p-0.5" title={t('kb.refluxExpand')}>
                  <Maximize2 className="w-3.5 h-3.5" />
                </button>
                <button onClick={closeRefluxModal} className="text-muted-foreground hover:text-foreground">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            <div className="px-4 py-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {refluxProgress && (
                      <span className="text-xs text-foreground truncate">
                        {t('kb.refluxProgress', { current: refluxProgress.current, total: refluxProgress.total, fileName: refluxProgress.fileName })}
                      </span>
                    )}
                  </div>
                  {refluxProgress && (
                    <div className="w-full h-1 bg-secondary rounded-full overflow-hidden">
                      <div className="h-full bg-primary rounded-full transition-all duration-300" style={{ width: `${(refluxProgress.current / refluxProgress.total) * 100}%` }} />
                    </div>
                  )}
                </div>
              )}
              {refluxResult && (
                <div className="flex items-center gap-4 text-xs">
                  <span className="text-green-500">{t('kb.refluxSuccess', { count: refluxResult.success })}</span>
                  {refluxResult.failed > 0 && <span className="text-red-500">{t('kb.refluxFailed', { count: refluxResult.failed })}</span>}
                </div>
              )}
            </div>
          </div>
      ...
      {/* Reflux Modal - Full (with backdrop) */}
      {refluxModalOpen && currentKb && !refluxMinimized && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => { if (!refluxLoading) closeRefluxModal(); }}>
          <div className="bg-background border rounded-lg shadow-lg w-[520px] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3 border-b">
              <h3 className="text-sm font-medium">{t('kb.batchReflux')}</h3>
              <div className="flex items-center gap-1">
                <button onClick={() => setRefluxMinimized(true)} className="text-muted-foreground hover:text-foreground p-0.5" title={t('kb.refluxMinimize')}>
              ...
              {refluxResult ? (
                <div className="space-y-3">
                  <div className="flex items-center gap-4 text-sm">
                    <span className="text-green-500">{t('kb.refluxSuccess', { count: refluxResult.success })}</span>
                    {refluxResult.failed > 0 && (
                      <span className="text-red-500">{t('kb.refluxFailed', { count: refluxResult.failed })}</span>
                    )}
                  </div>
                  {refluxResult.success > 0 && (
                    <div className="text-xs text-muted-foreground">{t('kb.refluxSubmitted')}</div>
                  )}
                  {refluxResult.errors.length > 0 && (
                    <div className="text-xs text-muted-foreground space-y-1">
                      {refluxResult.errors.map((err, i) => (
                        <div key={i}>{err}</div>
                      ))}
                    </div>
                  )}
            ...
            {!refluxLoading && !refluxResult && (
              <div className="px-4 py-3 border-t space-y-2">
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                    value={userAccount}
                    onChange={(e) => { setUserAccount(e.target.value); setUserAccountError(''); }}
                    placeholder={t('kb.refluxUserAccountPlaceholder')}
                    className="flex-1 px-2 py-1.5 text-xs border rounded bg-background focus:outline-none focus:ring-1 focus:ring-primary"
                  />
                  <button onClick={closeRefluxModal} className="px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">
                    {t('kb.cancel')}
                  </button>
                  <button onClick={handleReflux} disabled={refluxSelected.size === 0} className="px-3 py-1.5 text-xs bg-primary text-primary-foreground rounded hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                    {t('kb.refluxSubmit', { count: refluxSelected.size })}
                  </button>
                </div>
                {userAccountError && (
                  <div className="text-xs text-destructive">{t('kb.refluxUserAccountError')}</div>
                )}
              </div>
            )}
            {refluxResult && (
              <div className="flex items-center justify-end px-4 py-3 border-t">
                <button onClick={closeRefluxModal} className="px-3 py-1.5 text-xs bg-primary text-primary-foreground rounded hover:bg-primary/90 transition-colors">
                  {t('kb.close')}
                </button>
              </div>
            )}
